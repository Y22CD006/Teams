...
export function CreateChannelModal({ open, onClose, teamId, onCreated }: Props) {
      ...
      <div className="bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-2xl max-w-md w-full shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider font-mono">Channel Name</label>
            <input
              type="text"
              value={name}
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={isPrivate} onChange={(e) => setIsPrivate(e.target.checked)} className="rounded border-gray-600" />
            <span className="text-xs text-[var(--text-secondary)]">Private channel (only visible to selected members)</span>
          </label>
